...
- **没有真正的分布式**：池子、缓存、熔断状态都在单进程内存里。多实例部署时不共享。下一步：缓存挪到 Redis、熔断状态做成可共享。
- **没有限流**：第 7 步挡的是「内部资源被挤兑」，但没有针对客户端的 QPS 限流（令牌桶/漏桶），恶意刷量能打满。
- **没有压测数据**：很多参数（min/max、超时、队列上限、缓存大小）是拍脑袋定的，应用压测（autocannon / k6）反推合理值。
- **截图本身的健壮性**：懒加载、长页面、字体渲染、反爬，真实网页的坑还没系统处理。
- **下一步：Docker 容器化**（第 19 关），把跑环境固定下来。
