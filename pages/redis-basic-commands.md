...
1. 先学第 1～8 节，认识 key 和常用数据类型。
2. 再看第 9～10 节，复习易错点和常用命令。
3. 最后学第 11～15 节，理解开发中常见的进阶用法。
...
基础命令阶段已经完成。开发必学阶段共 6 关：

```text
第 1 关  缓存穿透、缓存击穿、缓存雪崩
第 2 关  Redis 与 MySQL 的数据一致性
第 3 关  分布式锁
第 4 关  大 key、热 key
第 5 关  持久化：RDB 与 AOF
第 6 关  高可用：主从复制、哨兵、集群
```

这一阶段从 0 / 6 单独计数，不影响已经完成的基础命令阶段。后续每学完一个小知识点，再把内容补充到对应章节。
